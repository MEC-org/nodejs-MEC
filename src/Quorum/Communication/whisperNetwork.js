...
function genesisConfigHandler(result, cb){
  let genesisPath = process.cwd() + './node_modules/mec/src/Quorum/quorum-genesis.json'
...
function staticNodesFileHandler(result, cb){
  let staticNodesPath = process.cwd() + `/Blockchain/${config.identity.nodeName}/static-nodes.json`
  let web3WSRPC = result.web3WSRPC;
  ...
  function onData(msg){
    ...
    if(message && message.indexOf(response.genesisConfig) >= 0){
      ...
      if(receivedGenesisConfig == false){
        ...
        fs.writeFile('./node_modules/mec/src/Quorum/quorum-genesis.json', genesisConfig, function(err, res){
          cb(err, result)
        })
      }
    }
  ...
  function onData(msg){
    ...
    if(message && message.indexOf(response.staticNodes) >= 0){
      ...
      if(receivedStaticNodesFile == false){ 
        ...
        fs.writeFile(`Blockchain/${config.identity.nodeName}/static-nodes.json`, staticNodesFile, function(err, res){
          cb(err, result)
        })
      }
    }
...
function startCommunicationNode(result, cb){
  ...
  var cmd = './node_modules/mec/src/Quorum/./startCommunicationNode.sh';
